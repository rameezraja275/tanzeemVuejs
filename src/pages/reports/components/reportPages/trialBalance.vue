<template>
  <div style="width: 100%; padding: 40px 30px;">
    ...
    <Table
      :currentHeaderForTable="headersForTrialBlnc"
      :isDefaultFooterHidden="false"
      :itemsForTable="itemsForTrialBalance"
...
import { fetchReportsTrialBlnc } from "../../actions/actions";
...
export default {
  ...
  methods: {
    ...
    prepareAndRenderDataInTable(data) {
      ...
      let difference = balanceForCr - balanceForDr;

      if (difference !== 0) {
        data.push({ diffOfCrDr: balanceForCr - balanceForDr });
      } else {
        data.push({ diffOfCrDr: "", icon: "mdi-check" });
      }
      this.itemsForTrialBalance = data;
    },
